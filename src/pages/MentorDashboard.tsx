...
const SessionsPage = () => <TutorSession />;
...
export default function TutorDashboard() {
  const [activeMenu, setActiveMenu] = useState<
    "dashboard" | "messages" | "sessions" | "students"
  >("dashboard");
  ...
  const openChatWithStudent = (student: Student) => {
    ...
    if (!chats.find((c) => c.id === student.id)) {
      setChats((prev) => [
        ...prev,
        { id: student.id, name: student.name, messages: [] },
      ]);
    }
  };
  ...
  const activeChat = selectedStudent
    ? chats.find((c) => c.id === selectedStudent.id) ?? {
        id: -1,
        name: "Unknown",
        messages: [],
      }
      ...
      case "dashboard":
        return (
          <>
            ...
            <RecentMessages
              messages={messages}
              onOpenChat={openChatWithStudent}
            />
          </>
        );
      ...
      case "messages":
        return activeChat ? (
          ...
          <div className="p-6 text-gray-600">
            Select a student from messages to start chatting.
          </div>
        );
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      <aside className="w-64 bg-white shadow p-4 flex flex-col">
        ...
        <nav className="space-y-2 flex-1">
          {(["dashboard", "messages", "sessions", "students"] as const).map(
            (menu) => (
              <button
                key={menu}
